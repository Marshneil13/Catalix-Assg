    ...
    <div className={`nav-container ${!isExpanded ? "hideNav" : "showNav"}`}>
      <div className="nav-logo">
        ...
        >
          <img
            ...
            width="40px"
            height="40px"
            className={`${!isExpanded && "rotateIcon"} revertIcon`}
          />
        </button>
        {isExpanded ? (
          <img src={mainLogo} alt="main-logo" />
        ) : (
          <img src={shortLogo} alt="main-logo" />
        )}
      </div>
      <div className={`nav-links ${!isExpanded ? "hideLinks" : "showLinks"}`}>
        {navItems.map((item) => (
          <Link
            to={item.path}
            className={`nav-items ${pathname === item.path && "active"}`}
      ...
      <div
        className={`nav-bottoms ${!isExpanded ? "hideBottoms" : "showBottoms"}`}
      >
        {navBottoms.map((item) => (
          <Link
            to={item.path}
